...
ZEPHIR_INIT_CLASS(Feathers_Request_Http_HttpRequest) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Feathers\\Request\\Http, HttpRequest, feathers, request_http_httprequest, feathers_request_requestabstract_ce, feathers_request_http_httprequest_method_entry, 0);

	zend_declare_property_null(feathers_request_http_httprequest_ce, SL("_rawBody"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_class_implements(feathers_request_http_httprequest_ce TSRMLS_CC, 1, feathers_request_http_httprequestinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Feathers_Request_Http_HttpRequest, isSoap) {
	...
	if (zephir_array_isset_string(server, SS("HTTP_SOAPACTION"))) {
		...
		if (zephir_array_isset_string_fetch(&contentType, server, SS("CONTENT_TYPE"), 0 TSRMLS_CC)) {
			RETURN_MM_BOOL(zephir_memnstr_str(contentType, SL("application/soap+xml"), "feathers/request/http/httpRequest.zep", 113));
		}
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Feathers_Request_Http_HttpRequest, getClientAddress) {
	...
	if (Z_TYPE_P(address) == IS_STRING) {
		if (zephir_memnstr_str(address, SL(","), "feathers/request/http/httpRequest.zep", 242)) {
			ZEPHIR_INIT_VAR(_0);
