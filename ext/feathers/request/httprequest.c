...
ZEPHIR_INIT_CLASS(Feathers_Request_HttpRequest) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Feathers\\Request, HttpRequest, feathers, request_httprequest, feathers_request_request_ce, feathers_request_httprequest_method_entry, 0);

	zend_declare_property_null(feathers_request_httprequest_ce, SL("_rawBody"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_class_implements(feathers_request_httprequest_ce TSRMLS_CC, 1, feathers_request_httprequestinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Feathers_Request_HttpRequest, isSoap) {
	...
	if (zephir_array_isset_string(server, SS("HTTP_SOAPACTION"))) {
		...
		if (zephir_array_isset_string_fetch(&contentType, server, SS("CONTENT_TYPE"), 0 TSRMLS_CC)) {
			RETURN_MM_BOOL(zephir_memnstr_str(contentType, SL("application/soap+xml"), "feathers/request/httpRequest.zep", 113));
		}
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Feathers_Request_HttpRequest, getClientAddress) {
	...
	if (Z_TYPE_P(address) == IS_STRING) {
		if (zephir_memnstr_str(address, SL(","), "feathers/request/httpRequest.zep", 242)) {
			ZEPHIR_INIT_VAR(_0);
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_1, &_0)
		...
		if (zephir_start_with_str(key, SL("HTTP_"))) {
			...
			object_init_ex(header, feathers_request_httprequestheader_ce);
			ZEPHIR_INIT_NVAR(_3);
...
PHP_METHOD(Feathers_Request_HttpRequest, getUploadedFiles) {
	...
	zend_bool onlySuccessful, _0, _4, _7, _8, _9;
		...
		if (zephir_array_isset_string_fetch(&error, file, SS("error"), 0 TSRMLS_CC)) {
			if (Z_TYPE_P(error) != IS_ARRAY) {
				...
				if (_4) {
					...
					object_init_ex(_5, feathers_request_httprequestfile_ce);
					if (zephir_has_constructor(_5 TSRMLS_CC)) {
						ZEPHIR_CALL_METHOD(NULL, _5, "__construct", NULL, file);
						zephir_check_call_status();
					}
					ZEPHIR_CALL_METHOD(NULL, filesCollection, "add", &_6, _5);
					zephir_check_call_status();
				}
			}
			...
			_7 = _4;
			if (_7) {
				_7 = zephir_array_isset_string_fetch(&type, file, SS("type"), 1 TSRMLS_CC);
			}
			_8 = _7;
			if (_8) {
				_8 = zephir_array_isset_string_fetch(&tmpName, file, SS("tmpName"), 1 TSRMLS_CC);
			}
			_9 = _8;
			if (_9) {
				_9 = zephir_array_isset_string_fetch(&size, file, SS("size"), 1 TSRMLS_CC);
			}
			if (_9) {
