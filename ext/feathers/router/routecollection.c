...
ZEPHIR_INIT_CLASS(Feathers_Router_RouteCollection) {
	...
	ZEPHIR_REGISTER_CLASS(Feathers\\Router, RouteCollection, feathers, router_routecollection, feathers_router_routecollection_method_entry, 0);

	zend_declare_property_null(feathers_router_routecollection_ce, SL("_routes"), ZEND_ACC_PRIVATE TSRMLS_CC);

	zend_class_implements(feathers_router_routecollection_ce TSRMLS_CC, 1, zend_ce_aggregate);
	zend_class_implements(feathers_router_routecollection_ce TSRMLS_CC, 1, spl_ce_Countable);
	return SUCCESS;
...
PHP_METHOD(Feathers_Router_RouteCollection, add) {
	...
	if (_0) {
		ZEPHIR_THROW_EXCEPTION_STR(zend_exception_get_default(TSRMLS_C), "Routes must have a name.");
		return;
	}
	_1 = zephir_fetch_nproperty_this(this_ptr, SL("_routes"), PH_NOISY_CC);
	if (zephir_array_isset(_1, routeName)) {
		ZEPHIR_THROW_EXCEPTION_STR(zend_exception_get_default(TSRMLS_C), "A Route was already defined with the same name.");
		return;
	}
	zephir_update_property_array(this_ptr, SL("_methods"), routeName, route TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
