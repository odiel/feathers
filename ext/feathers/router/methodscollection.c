...
ZEPHIR_INIT_CLASS(Feathers_Router_MethodsCollection) {
	...
	ZEPHIR_REGISTER_CLASS(Feathers\\Router, MethodsCollection, feathers, router_methodscollection, feathers_router_methodscollection_method_entry, 0);

	zend_declare_property_null(feathers_router_methodscollection_ce, SL("_methods"), ZEND_ACC_PRIVATE TSRMLS_CC);

	zend_class_implements(feathers_router_methodscollection_ce TSRMLS_CC, 1, zend_ce_aggregate);
	zend_class_implements(feathers_router_methodscollection_ce TSRMLS_CC, 1, spl_ce_Countable);
	return SUCCESS;
